...
impl<'a> UnaryOp<'a> {
    ...
    /// Address-of (`&expr`): returns the raw pointer without loading the operand.
    fn visit_ref<'ctx>(
        &self,
        context: &mut LLVMContext<'ctx>,
    ) -> CodegenResult<CodegenValue<'ctx>> {
        let ptr = context.get_ptr(self.operand)?;
        let ptr_type = Types::Pointer(Box::new(self.operand.get_type()));
        Ok(CodegenValue::new(ptr.into(), ptr_type))
    }

    /// Dereference (`*expr`): loads the value the pointer points to.
    fn visit_deref<'ctx>(
        &self,
        context: &mut LLVMContext<'ctx>,
    ) -> CodegenResult<CodegenValue<'ctx>> {
    ...
    /// All other unary operators: negation, logical/bitwise not.
    fn visit_arithmetic<'ctx>(
        &self,
        context: &mut LLVMContext<'ctx>,
    ) -> CodegenResult<CodegenValue<'ctx>> {
        let operand_val = self.operand.visit(context)?;
        let operand_type = operand_val.unwrap_concrete()?.type_info();

        let result_type = context
            .symbols()
            .infer_unary_type(self.op, operand_type, Span::default())
            .map_err(|_| CodegenError::TypeMismatch {
                expected: format!("{:?} {:?}", self.op, operand_type),
                actual: "incompatible type".to_string(),
            })?;

        let builder = context.builder();
        let result_val: BasicValueEnum = match (operand_val.unwrap_concrete()?.type_info(), self.op)
        {
...
impl<'a> Visit for UnaryOp<'a> {
    ...
    fn visit<'ctx>(&self, context: &mut LLVMContext<'ctx>) -> CodegenResult<Self::Output<'ctx>> {
        match self.op {
            Op::Ref => self.visit_ref(context),
            Op::Deref => self.visit_deref(context),
            _ => self.visit_arithmetic(context),
        }
    }
}
